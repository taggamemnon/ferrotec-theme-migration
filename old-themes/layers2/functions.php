...
function themes_scripts () {
	...
	wp_enqueue_style('layers-style', get_template_directory_uri() . '/style.css' , array('bootstrap'), '2.1' );
	wp_enqueue_style('pa-style', get_template_directory_uri() . '/pa.css' , array('bootstrap', 'layers-style'), '2.2' );
	wp_enqueue_script(
		'bootstrap',
	...
	wp_enqueue_script(
		'float-panel',
		get_template_directory_uri() . '/js/float-panel.js',
		array(),
		'',
		true
	);

	wp_enqueue_script('modernizr', get_template_directory_uri() . '/js/vendor/modernizr-2.6.2-respond-1.1.0.min.js', array(), '' , false);
...
function post_type_block_desc( $views ){
...
add_filter("views_edit-auc_block", 'post_type_block_desc');
...
function auc_create_post_type () {
	register_post_type('auc_block', array(
		...
		'description' =>'Used for reusable content blocks in pages. To show a block use shortcode: [show_block id={post_id}]'
	));
...
function show_homepage_pr ($atts, $content = null) {
    $a = shortcode_atts( array(
        'items' => 6,
    ), $atts );
	$output = '';
	...
	if ($query->have_posts()) :
		$output .= '<ul class="news_home">';
		while ($query->have_posts()) : $query->the_post();
			$output .= '<li><a href="' . get_the_permalink() . '">';
			$output .= '<div class="news-cat">Press release</div>';
			$output .= '<div class="news-title"><span class="news-date">'. get_the_date() .'</span> | ' . get_the_title() .'</div>';
			$output .= '</a></li>';
		endwhile;
		$output .= '</ul>';
	endif;
...
add_shortcode('show_homepage_pr', 'show_homepage_pr');
...
add_filter( 'gform_field_value_setgclid', 'populate_gclid' );
		...
		if ($pline_set_data){
			...
			foreach ($pline_set_data as $product ){
				if($product->model !== "APG 859"){ // discontinued item
				  ...
				  $output .= '    <td><a href="'. makeurls($product->model) . ($product->model == "EMG 700" ? "-sp" : '') . '">'. $product->model . ($product->model == "EMG 700" ? " SP" : '') . '</a></td>';
...
function show_ferrofluid_products($atts, $content = null){
    $a = shortcode_atts( array(
        'family' => '',
    ), $atts );
	$output = '';
	$q = get_posts(
		...
		if ($q){
			...
			foreach ($q as $p ){
				  $output .= '<tr class="ferrofluid-listing">';
				  $output .= '    <td><a href="'. makeurls( get_field('model', $p) ). '">'. get_field('model', $p) . '</a></td>';
				  $output .= '    <td>'. get_field('sat_guass', $p) .'</td>';
				  $output .= '    <td>'. get_field('sat_mt', $p) .'</td>';
				  $output .= '    <td>'. get_field('vis_cp', $p) .'</td>';
				  $output .= '    <td>'. get_field('sat_mpa_s', $p) .'</td>';
				  $output .= '</tr>';
			}
			$output .= '</tbody>';
			$output .= '</table>';
...
add_shortcode('ferrofluid_products', 'show_ferrofluid_products');
...
add_filter( 'gform_submit_button', 'add_reCaptcha_to_submit', 10, 2 );
function add_reCaptcha_to_submit( $button, $form ) {
    // Return without changes for the admin back-end.
    if ( is_admin() ){
        return $button;
    }
    $btn = '<div class="gcaptcha-wrapper">
        <div class="gcaptcha-graphic">
            <img decoding="async" src="/wp-content/themes/layers2/img/recaptcha3-graphic.png" alt="Google regcaptcha logo">
        </div>
        <div class="gcaptcha-text-wrapper">
            <div class="gcaptcha-title">Protected by reCAPTCHA</div>
            <div class="gcaptcha-links">
