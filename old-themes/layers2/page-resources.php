			...
			<div class="col-sm-12">
				<ul class="nav nav-tabs js-tabCollapse-sm" role="tablist" id="resources-tabs">
					...
					foreach ($terms as $k => $v) :
						...
						if ($resources->have_posts()) :
							$row_count ++; ?>
							<li class="<?php echo($row_count == 1 ? 'active' : ''); ?>">
								<a href="<?php echo '#' . sanitize_title_with_dashes($k); ?>"
								   role="tab"
								   data-toggle="tab"><?php echo $v . 's'; ?></a></li>
						<?php endif; ?>
					<?php endforeach; ?>
				</ul>
				<div class="tab-content">
					...
					foreach ($terms as $k => $v) :
						...
						if ($resources->have_posts()) :
							$row_count ++; ?>
							<div class="tab-pane <?php echo($row_count == 1 ? 'active' : ''); ?>"
							     id="<?php echo sanitize_title_with_dashes($k); ?>">
											...
											<div class="resource-tile <?php echo $v; ?>-tile">
												...
												<?php if (get_field('gated')) : ?>
													<form action="<?php echo site_url('/register') ?>" method="post">
														...
														<button type="submit"><?php echo(in_array($k, array(
																'webinar',
																'video'
													...
													<?php if (in_array($k, array('webinar', 'video'))) : ?>
														<a href="<?php the_permalink(); ?>">
															<?php if (get_field('video_thumbnail')) { ?>
																<img src="<?php the_field('video_thumbnail'); ?>" alt="thumbnail preview">
																<br>
															<?php } ?>
															Watch</a>
													<?php else : ?>
