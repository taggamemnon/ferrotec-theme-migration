...
//$sql = "select * from teCoeff  where id = {$id} ";
...
if($rst && count($rst) > 0)
{
  ...
  for($j=0;$j<=($tmax-$tmin)/$tg;$j++)
  {
    $data= Array();
    $cols[]=  "dT=".($j*$tg+$tmin);
    foreach($curve[$j] as $k => $xydata)
    {  $data[]=$xydata[1]; if($xydata[1]>$ymax)  $ymax=$xydata[1]; $rows[$k][] = $xydata[1];  }
    $plot[$j]['datay'] = $data;
    $plot[$j]['datax'] = $xdata; 
...
$dataTable = array_merge( Array ($cols) , $rows);
